...
library(reshape2) # install.packages("tidyverse")
```
...
```{r}
ggplot(buoy_daily, aes(date,mean_WSPD_m_s_clean, colour="red")) +
  geom_point() +
  # theme(axis.text.x = element_text(angle = 90)) + # rotate x-axis labels 90 degrees
  geom_point(aes(date,mean_WTMP_degC_clean),
            show_guide = FALSE, colour="blue") +
  theme(legend.position = c(.1, .9)) +
  labs(x="Month", y="Temp (degC) or Wind Speed (m/s)", col="data") +
  ggtitle("2017 M1 Buoy - Mean Daily Wind Speed and Temperature") +
  scale_x_date(date_minor_breaks = "1 week", limits = as.Date(c('2017-05-01','2017-09-01'))) +
  scale_color_manual(labels = c("wind", "temp"), values = c("red","blue")) 
